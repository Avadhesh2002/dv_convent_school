...
// Preview: 57×87mm ÷ 3.5 → 163×249
const PW = 163, PH = 249;
const pp = (mm) => Math.round(mm * PW / 57);
...
const PremiumCard = ({ theme, gold='#c9a94a', logoSrc, schoolName, schoolAddress,
  schoolPhone, cardLabel, photo, personName, idLine, rows }) => {

  const ribbonH = pp(8), headerH = pp(26), photoZH = pp(35), footerH = pp(9);
  const pzY  = ribbonH + headerH;
  const infoY = pzY + photoZH;
  const infoH = PH - ribbonH - headerH - photoZH - footerH;
  const ftY  = PH - footerH;
  const pW   = pp(22), pH = pp(27);
  const pX   = (PW - pW) / 2, pY = pzY + pp(3.5);

  return (
    <div style={{ width:PW, height:PH, position:'relative', fontFamily:'Arial,sans-serif',
      borderRadius:6, overflow:'hidden', flexShrink:0,
      boxShadow:'0 10px 40px rgba(0,0,0,0.28), 0 2px 8px rgba(0,0,0,0.15)' }}>

      {/* ── Ribbon space ── */}
      <div style={{ position:'absolute', top:0, left:0, right:0, height:ribbonH,
        background:`linear-gradient(180deg,${theme}28 0%,#fff 100%)`,
        display:'flex', alignItems:'center', justifyContent:'center' }}>
        {/* metallic hole */}
        <div style={{ width:pp(5.2), height:pp(5.2), borderRadius:'50%',
          background:`radial-gradient(circle at 35% 35%, #e8d88a, #a07830)`,
          boxShadow:'0 1px 4px rgba(0,0,0,0.3)', display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ width:pp(3.8), height:pp(3.8), borderRadius:'50%', background:'#c8cedf',
      ...
      <div style={{ position:'absolute', top:ribbonH, left:0, right:0, height:headerH,
        background:`linear-gradient(120deg, ${theme} 0%, ${theme}dd 100%)`,
        overflow:'hidden' }}>
        {/* geometric lines */}
        <svg style={{ position:'absolute', inset:0, width:'100%', height:'100%', opacity:0.15 }}>
          {Array.from({length:12},(_,i)=>(
            <line key={i} x1={i*14-20} y1="0" x2={i*14+22} y2="100%" stroke={gold} strokeWidth="0.8"/>
          ))}
          <line x1="0" y1="30%" x2="100%" y2="30%" stroke={gold} strokeWidth="0.5" opacity="0.6"/>
          <line x1="0" y1="70%" x2="100%" y2="70%" stroke={gold} strokeWidth="0.5" opacity="0.6"/>
        </svg>
        {/* top gold line */}
        <div style={{ position:'absolute', top:0, left:'10%', right:'10%', height:1.5,
          background:`linear-gradient(90deg,transparent,${gold},transparent)` }} />
        {/* bottom gold line */}
        <div style={{ position:'absolute', bottom:0, left:'10%', right:'10%', height:1.5,
          background:`linear-gradient(90deg,transparent,${gold},transparent)` }} />

        {/* Logo */}
        <div style={{ position:'absolute', left:pp(3.5), top:'50%', transform:'translateY(-50%)',
          width:pp(14), height:pp(14), borderRadius:'50%',
          background:`linear-gradient(135deg,#e8d88a,#a07830)`,
          padding:1.5, boxShadow:`0 0 ${pp(3)}px ${gold}88` }}>
        ...
        {/* School name */}
        <div style={{ position:'absolute', left:pp(20), right:pp(3), top:pp(3) }}>
          <div style={{ color:'#fff', fontWeight:900, fontSize:pp(4), lineHeight:1.2,
            textShadow:'0 1px 3px rgba(0,0,0,0.3)' }}>{schoolName}</div>
          {/* gold divider */}
          <div style={{ height:1, background:`linear-gradient(90deg,${gold},transparent)`,
            margin:`${pp(1.5)}px 0`, width:'80%' }} />
          <div style={{ color:'rgba(255,255,255,0.72)', fontSize:pp(2.5) }}>{schoolAddress}</div>
          <div style={{ color:gold+'ee', fontWeight:700, fontSize:pp(2.7), marginTop:pp(1) }}>
            Ph: {schoolPhone}
          </div>
          {/* badge */}
          <div style={{ display:'inline-block', marginTop:pp(1.5),
            background:`linear-gradient(90deg,${gold},#e8c84a)`,
            color:theme, fontWeight:900, fontSize:pp(2.3),
            padding:`${pp(0.8)}px ${pp(2.5)}px`, borderRadius:pp(2) }}>
            {cardLabel}
          </div>
        </div>
      </div>
      ...
      {/* ── Photo zone ── */}
      <div style={{ position:'absolute', top:pzY, left:0, right:0, height:photoZH,
        background:'linear-gradient(180deg,#f0f3ff 0%,#fff 100%)' }}>

        {/* Photo with gold frame */}
        <div style={{ position:'absolute', left:pX-pp(1.5), top:pY-pp(1.5),
          width:pW+pp(3), height:pH+pp(3),
          background:`linear-gradient(135deg,#e8d88a 0%,#c9a94a 25%,#f0e090 50%,#c9a94a 75%,#a07830 100%)`,
          borderRadius:pp(1.5), boxShadow:`0 ${pp(2)}px ${pp(5)}px rgba(0,0,0,0.2)` }} />
        <div style={{ position:'absolute', left:pX, top:pY, width:pW, height:pH,
          overflow:'hidden', borderRadius:pp(1), background:'#dbeafe' }}>
          {photo
            ? <img src={photo} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', objectPosition:'center top' }} />
            : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center',
                fontSize:pp(12), fontWeight:900, color:theme }}>{personName?.charAt(0)}</div>}
        </div>

        {/* Name */}
        <div style={{ position:'absolute', bottom:pp(6), left:0, right:0, textAlign:'center',
          fontWeight:900, fontSize:pp(4.2), color:theme,
          overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', padding:`0 ${pp(3)}px` }}>
          {personName}
        </div>

        {/* UID pill */}
        <div style={{ position:'absolute', bottom:pp(1), left:'50%', transform:'translateX(-50%)',
          background:`linear-gradient(90deg,${theme},${theme}bb)`,
          color:'#fff', fontWeight:800, fontSize:pp(2.8),
          padding:`${pp(1)}px ${pp(4)}px`, borderRadius:pp(3),
          border:`0.8px solid ${gold}66`,
          boxShadow:`0 ${pp(1)}px ${pp(3)}px ${theme}55`, whiteSpace:'nowrap' }}>
          {idLine}
        </div>
      </div>

      {/* ── Info section ── */}
      <div style={{ position:'absolute', top:infoY, left:0, right:0, height:infoH,
        background:'#fff', overflow:'hidden' }}>
        {/* top gold line */}
        <div style={{ height:1.5, background:`linear-gradient(90deg,transparent 5%,${gold} 20%,${gold} 80%,transparent 95%)` }} />
        {rows.map(([lbl,val],i)=>(
          <div key={lbl} style={{ display:'flex', alignItems:'flex-start',
            padding:`${pp(0.8)}px ${pp(3.5)}px`,
            background:i%2===0?theme+'06':'transparent',
            borderBottom:`0.5px solid #e5e7eb` }}>
            {/* left accent */}
            <div style={{ width:pp(0.8), alignSelf:'stretch', background:`linear-gradient(180deg,${theme}cc,${theme}33)`,
              marginRight:pp(2), borderRadius:pp(0.5), flexShrink:0 }} />
            <span style={{ fontSize:pp(2.8), fontWeight:700, color:theme, width:pp(14), flexShrink:0 }}>{lbl}</span>
            <span style={{ fontSize:pp(2.8), color:'#6b7280', marginRight:pp(1.5) }}>:</span>
      ...
      <div style={{ position:'absolute', bottom:0, left:0, right:0, height:footerH,
        background:`linear-gradient(90deg,${theme}f0,${theme})`,
        display:'flex', alignItems:'center', justifyContent:'space-between',
        padding:`0 ${pp(3.5)}px` }}>
        {/* top gold line */}
        <div style={{ position:'absolute', top:0, left:'10%', right:'10%', height:1.2,
          background:`linear-gradient(90deg,transparent,${gold},transparent)` }} />
        <span style={{ fontSize:pp(2.5), fontWeight:700, color:'rgba(255,255,255,0.9)' }}>
          📞 {schoolPhone}
        </span>
        <div style={{ textAlign:'center' }}>
          <img src={signImage} alt="" style={{ height:pp(5.5), objectFit:'contain', display:'block', margin:'0 auto' }}
            onError={e=>{e.target.style.display='none';}} />
          <div style={{ fontSize:pp(2.3), color:gold+'dd', fontWeight:700 }}>Principal</div>
        </div>
      </div>
    </div>
  );
